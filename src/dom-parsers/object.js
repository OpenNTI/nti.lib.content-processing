...
function addValueFor(o, n, v) {
	...
	let c = o[n];
	o[n] = c ? (Array.isArray(c) ? c : [c]).concat(v) : v;
}

function getDirectChildNodes(el, tag) {
	tag = tag.toUpperCase();
	return Array.from(el.childNodes).filter(
		node => node.nodeName.toUpperCase() === tag
	);
}
...
export default function parseDomObject(el) {
	...
	Array.from(el.attributes).forEach(p => {
		addValueFor(obj, hyphenatedToCamel(p.name), p.value);
	});

	getDirectChildNodes(el, 'param').forEach(p =>
		addValueFor(obj, p.name, p.value)
	);

	obj.children = getDirectChildNodes(el, 'object').map(p =>
		parseDomObject(p)
	);
